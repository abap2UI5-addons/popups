...
CLASS z2ui5_cl_popup_to_select DEFINITION LOCAL FRIENDS ltcl_test.
...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_to_select=>factory( lt_tab ).
  ...
  METHOD test_factory_multi.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_to_select=>factory(
      i_tab         = lt_tab
  ...
  METHOD test_factory_default_title.
    ...
    DATA(lo_single) = z2ui5_cl_popup_to_select=>factory( lt_tab ).
    DATA(lo_multi)  = z2ui5_cl_popup_to_select=>factory(
      i_tab         = lt_tab
      i_multiselect = abap_true ).
...
CLASS ltcl_test_roundtrip DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS popup_create
      RETURNING
        VALUE(ro_pop) TYPE REF TO z2ui5_cl_popup_to_select.

    METHODS select_row
      IMPORTING
        io_pop   TYPE REF TO z2ui5_cl_popup_to_select
        iv_index TYPE i.
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD popup_create.
    ...
    DATA(lt_tab) = VALUE ty_t_tab( ( name = `alpha` count = 1 )
                                   ( name = `bravo` count = 2 ) ).
    ro_pop = z2ui5_cl_popup_to_select=>factory( i_tab             = lt_tab
