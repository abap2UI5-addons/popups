CLASS z2ui5_cl_popup_sample_154 DEFINITION PUBLIC.
...
CLASS z2ui5_cl_popup_sample_154 IMPLEMENTATION.
  ...
  METHOD on_event.
    ...
    CASE client->get( )-event.
      ...
      WHEN `POPUP_BAPIRET`.
        ...
        client->nav_app_call( z2ui5_cl_popup_messages=>factory( lt_msg ) ).

      WHEN `POPUP_BALLOG`.

        lt_bal = VALUE #(
          ( msgid = `MSG1` msgno = `001` msgty = `S` time_stmp = z2ui5_cl_abap2ui5_context=>time_get_timestampl( ) msgnumber = `01` )
          ( msgid = `MSG2` msgno = `002` msgty = `S` time_stmp = z2ui5_cl_abap2ui5_context=>time_get_timestampl( ) msgnumber = `02` ) ).

        client->nav_app_call( z2ui5_cl_popup_messages=>factory( lt_bal ) ).
