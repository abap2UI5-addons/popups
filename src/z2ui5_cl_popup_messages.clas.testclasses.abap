...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory_w_bapiret.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory( lt_msg ).
  ...
  METHOD test_factory_w_cx.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory( lx ).
    cl_abap_unit_assert=>assert_bound( lo_pop ).
  ...
  METHOD test_factory_empty_input.
    ...
    DATA(lx) = NEW z2ui5_cx_popup_error( val = `test` ).
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory( lx ).
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD test_factory_maps_messages.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory(
  ...
  METHOD test_init_displays_popup.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory( i_messages = VALUE string_table( ( `first message` ) )
                                                   i_title    = `MSG Title` ).
  ...
  METHOD test_continue_closes.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_messages=>factory( VALUE string_table( ( `msg` ) ) ).
