...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory_table.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_data=>factory( lt_tab ).
  ...
  METHOD test_factory_struc.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_data=>factory( ls_data ).
  ...
  METHOD test_factory_title.
    ...
    DATA(lo_pop) = z2ui5_cl_popup_data=>factory( val   = lt_tab
                                               title = `My Data` ).
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD test_init_table_calls_popup.
    ...
    DATA(lo_table_pop) = CAST z2ui5_cl_popup_table( mo_action->ms_next-o_app_call ).
  ...
  METHOD test_init_struct_calls_popup.
    ...
    DATA(ls_row) = VALUE ty_s_row( name  = `row1`
                                   count = 1 ).
    DATA(lo_pop) = z2ui5_cl_popup_data=>factory( ls_row ).
    client_create( lo_pop ).
